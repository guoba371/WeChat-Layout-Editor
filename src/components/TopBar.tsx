...
const toolbarItems = [
  ...
  { label: '无序列表', icon: List, command: 'unordered-list' as const },
  { label: '有序列表', icon: ListOrdered, command: 'ordered-list' as const },
  { label: '插入图片', icon: Image, command: 'image' as const },
  { label: '分割线', icon: Minus, command: 'divider' as const },
];
...
function TopBar({ isDark, activeEditorState, onToggleDark, onEditorCommand }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 grid h-[72px] grid-cols-[270px_minmax(420px,1fr)_270px] items-center border-b border-[var(--border)] bg-white/80 px-6 backdrop-blur-[18px] dark:border-white/10 dark:bg-[#0a0b0c]/75">
      <div className="flex items-baseline gap-4">
        <span className="text-[23px] font-black tracking-normal text-black dark:text-white">SaiWechatEdit</span>
        <span className="font-serif text-[13px] italic tracking-[0.28em] text-slate-400">
          灵感流转，由此开始
        </span>
      </div>
      ...
      <nav
        aria-label="排版工具栏"
        className="justify-self-center rounded-[18px] border border-slate-200/70 bg-white/75 px-3 py-1.5 shadow-sm backdrop-blur-[18px] dark:border-white/10 dark:bg-black/55"
      >
        <div className="flex h-10 items-center gap-1 text-slate-500 dark:text-white/55">
          <div className="relative">
            <button
              ...
              className={[
                'inline-flex h-10 min-w-16 items-center justify-center gap-1 rounded-xl px-2 text-sm transition',
            ...
            {headingMenuOpen ? (
              <div className="absolute left-0 top-12 z-30 w-36 overflow-hidden rounded-2xl border border-slate-200/80 bg-white/95 p-1.5 shadow-xl backdrop-blur-[18px] dark:border-white/10 dark:bg-[#111]/95">
                {headingOptions.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        'grid h-9 w-full grid-cols-[1fr_auto] items-center rounded-xl px-2 text-left text-xs transition',
                        selected
              ...
              <span key={item.label} className="group relative inline-flex">
                ...
                {'tooltip' in item ? (
                  <span
                    ...
                    role="tooltip"
                    className="pointer-events-none absolute left-1/2 top-12 z-40 w-max -translate-x-1/2 translate-y-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[12px] font-medium text-slate-700 opacity-0 shadow-lg transition duration-150 group-hover:translate-y-0 group-hover:opacity-100 dark:border-white/10 dark:bg-[#161616] dark:text-white/80"
                  >
                    {item.tooltip}
                  </span>
                ) : null}
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          ...
          className="inline-flex h-10 w-10 items-center justify-center rounded-xl text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 dark:text-white/60 dark:hover:bg-white/10 dark:hover:text-white"
        >
          {isDark ? <Sun size={18} /> : <Moon size={18} />}
        </button>
        <button
          type="button"
          className="inline-flex h-10 items-center justify-center rounded-[15px] border border-slate-200 bg-white px-4 text-sm font-medium text-slate-600 shadow-sm transition hover:text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-white/65 dark:hover:text-white"
        >
          登录
        </button>
      </div>
